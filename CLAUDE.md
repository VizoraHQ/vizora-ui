...
*Last refreshed: 2026-05-30 — Playground is a **single upload-driven Data Explorer** (upload → visualize → filter → **aggregate** → AI chat) on branch `feat/data-explorer`. Latest: added a group-by **aggregation engine + Pareto contribution insight** ("ask where the money went" → ranked bars + a deterministic "% of the total" headline), wired to the AI `aggregate` action; `avg`/blank-cell and negative-measure correctness fixed post-review (commits `0298235`, `1689ee6`, pushed). Earlier on this branch: removed the seeded "AI Ops dashboard", the nav rail, `src/data.ts`, and the legacy `src/upload/` flow; theme switcher moved into the explorer's left sidebar; `papaparse` + `@types/papaparse` added. typecheck/build pass; verified live in-browser across all three themes. v0.2 charts (Pie/Donut/Scatter/Heatmap) already in main; Radar still parked. Repo at VizoraHQ/vizora-ui. CI green.*
